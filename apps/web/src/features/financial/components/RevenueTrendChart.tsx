...
import { getStoredSettings, formatCurrencyValue } from "@/shared/lib/settingsStore";
...
const DEFAULT_REVENUE_BARS: RevenueChartBar[] = [
  { label: "Jan", val: 28, revenueText: "Rp 28.4 M", heightPercent: 62 },
  { label: "Feb", val: 31, revenueText: "Rp 31.2 M", heightPercent: 70 },
  { label: "Mar", val: 29, revenueText: "Rp 29.8 M", heightPercent: 64 },
  { label: "Apr", val: 34, revenueText: "Rp 34.5 M", heightPercent: 78 },
  { label: "Mei", val: 36, revenueText: "Rp 36.1 M", heightPercent: 82 },
  { label: "Jun", val: 38, revenueText: "Rp 38.1 M", heightPercent: 86 },
  { label: "Jul", val: 42, revenueText: "Rp 42.8 M", heightPercent: 100 },
  { label: "Agu", val: 39, revenueText: "Rp 39.4 M", heightPercent: 91 },
  { label: "Sep", val: 37, revenueText: "Rp 37.8 M", heightPercent: 85 },
  { label: "Okt", val: 41, revenueText: "Rp 41.2 M", heightPercent: 94 },
  { label: "Nov", val: 35, revenueText: "Rp 35.8 M", heightPercent: 80 },
  { label: "Des", val: 40, revenueText: "Rp 40.5 M", heightPercent: 92 },
];

export const RevenueTrendChart: React.FC = () => {
  const [hoveredIdx, setHoveredIdx] = useState<number | null>(null);
  const [filterPeriod, setFilterPeriod] = useState<"Bulanan" | "Kuartal">("Bulanan");
  const [bars, setBars] = useState<RevenueChartBar[]>(DEFAULT_REVENUE_BARS);
  ...
  useEffect(() => {
    if (filterPeriod === "Kuartal") {
      setBars([
        { label: "Q1", val: 88, revenueText: "Rp 89.4 M", heightPercent: 72, subtitle: "Jan - Mar" },
        { label: "Q2", val: 108, revenueText: "Rp 112.7 M", heightPercent: 88, subtitle: "Apr - Jun" },
        { label: "Q3", val: 118, revenueText: "Rp 120.0 M", heightPercent: 100, subtitle: "Jul - Sep" },
        { label: "Q4", val: 115, revenueText: "Rp 117.5 M", heightPercent: 94, subtitle: "Okt - Des" },
      ]);
    } else {
      setBars(DEFAULT_REVENUE_BARS);
    }
  }, [filterPeriod]);

  const getFormattedValue = (rawText: string) => {
    return formatCurrencyValue(rawText, settings.currency);
  };
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 mb-4 sm:mb-6">
        <div>
          <span className="text-[10px] sm:text-[11px] font-semibold text-textGray-muted uppercase tracking-[0.08em] block mb-1">
            PERFORMA P&L & CASH FLOW REAL-TIME
          </span>
          <h3 className="text-[18px] font-display font-semibold text-textGray-display leading-tight">
            {filterPeriod === "Bulanan" ? "Tren Pendapatan Bulanan 2026" : "Akumulasi Revenue Per Kuartal 2026"}
          </h3>
        </div>
        ...
        >
          <AnimatePresence mode="wait">
            {bars.map((bar, idx) => {
              ...
              const formattedRev = getFormattedValue(bar.revenueText);
