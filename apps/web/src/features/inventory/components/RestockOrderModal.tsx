...
import { X, Plus, CheckCircle2, Sparkles, ChevronDown, Package } from "lucide-react";
...
export const RestockOrderModal: React.FC<RestockOrderModalProps> = ({ isOpen, onClose, onSuccess }) => {
  ...
  const [priority, setPriority] = useState("Prioritas Tinggi (3 Hari)");
              ...
              <div className="flex items-center gap-2.5">
                ...
                <div>
                  <h3 className="text-[17px] font-display font-bold text-textGray-display leading-tight">
                    Order Restock Inventaris
                  </h3>
                  <p className="text-[12px] text-textGray-tertiary font-normal">
                    Buat pengajuan alokasi unit baru ke cabang tujuan
                  </p>
                </div>
              </div>
            ...
            {isSubmitted ? (
              <div className="py-8 flex flex-col items-center justify-center text-center gap-3">
                ...
                <h4 className="text-[18px] font-bold text-textGray-display">
                  Restock Order Berhasil Diajukan!
                </h4>
                <p className="text-[13px] text-textGray-tertiary max-w-[320px]">
                  Permintaan restock <span className="font-semibold text-textGray-display">{quantity} unit {brand} {model}</span> ke cabang <span className="font-semibold text-textGray-display">{branch}</span> telah diproses.
                </p>
              </div>
            ) : (
              /* Form Fields */
                ...
                <div className="flex flex-col gap-1.5">
                  ...
                  <div className="relative">
                    <select
                      ...
                      className="w-full appearance-none px-3.5 py-2.5 pr-9 rounded-xl bg-surfaceLight-pearl border border-surfaceLight-border text-textGray-display text-[13.5px] focus:outline-none focus:border-[#4B8E55] transition-colors cursor-pointer"
                    >
                      <option value="Prioritas Tinggi (3 Hari)">Prioritas Tinggi (3 Hari Express)</option>
                      <option value="Standar (7 Hari)">Standar (7 Hari Kerja)</option>
                      <option value="Alokasi Bulanan">Alokasi Bulanan</option>
                    </select>
                    <ChevronDown className="absolute right-3 top-1/2 -translate-y-1/2 w-4 h-4 text-textGray-tertiary pointer-events-none" strokeWidth={1.5} />
                  </div>
                </div>
